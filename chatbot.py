    ...
    if user_question:
        ...
        if relevant_chunks:
            ...
            for chunk in relevant_chunks:
                # Simple sentence splitting without regex
                sentences = []
                for sep in ['.', '!', '?']:
                    chunk = chunk.replace(sep, '|SPLIT|')
            ...
            if best_answers:
                ...
                # Highlight key words in the chunk (simple approach)
                highlighted_chunk = chunk
                for word in key_words:
                    if len(word) > 2:  # Only highlight meaningful words
                        highlighted_chunk = highlighted_chunk.replace(word, f'**{word}**')
                        highlighted_chunk = highlighted_chunk.replace(word.capitalize(), f'**{word.capitalize()}**')
